...
import { updateRepoStats } from './githubCache.queries'
...
describe('githubCache', () => {
  describe('updateRepoStats', () => {
    it('should create a new github release entry', async () => {
      ...
      const result = await updateRepoStats.run(
        {
          github_id: 123456,
          repo_id: repoId,
    ...
    it('should update an existing github release entry', async () => {
      ...
      const initialRelease = await updateRepoStats.run(
        {
          github_id: 123456,
          repo_id: repoId,
          releaseCreatedAt: new Date('2023-01-01'),
      ...
      const updatedRelease = await updateRepoStats.run(
        {
          github_id: 123456,
          repo_id: repoId,
    ...
    it('should handle multiple releases for the same repo with different ids', async () => {
      ...
      const firstRelease = await updateRepoStats.run(
        {
          github_id: 123456,
          repo_id: repoId,
          releaseCreatedAt: new Date('2023-01-01'),
      ...
      const secondRelease = await updateRepoStats.run(
        {
          github_id: 789012,
          repo_id: repoId,
